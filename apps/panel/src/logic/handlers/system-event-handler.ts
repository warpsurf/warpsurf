...
import { updateWorkerProgress, handleWorkerTabCreated, handleSingleAgentTabCreated, updateTabGroupColor,
  reconstructWorkerTabGroupsFromPreview, shouldShowCloseTabs,
  addRunningAgent, moveToCompleted } from './utils';
import { parseJobSummary, storeJobSummary, updateRequestSummary, updateLastAgentMessageSummary,
  updateAggregateRootSummary, addTraceItem, markAggregateComplete, persistFinalPreview } from './utils';
...
export const createSystemHandler: EventHandlerCreator = (deps) => {
  ...
  return (event) => {
    ...
    try { updateWorkerProgress(event, deps); } catch {}
    ...
    switch (state) {
      ...
      case ExecutionState.TASK_OK:
        ...
        try { if (deps.portRef.current?.name === 'side-panel-connection') deps.portRef.current.postMessage({ type: 'get-all-mirrors-for-cleanup' }); } catch {}
        try {
          const taskId = String(data?.taskId || deps.sessionIdRef.current || '');
        ...
        const isCurrentlyBrowserUse = deps.getCurrentTaskAgentType() === 'agent' || deps.getCurrentTaskAgentType() === 'agent';
        if (deps.agentTraceRootIdRef.current && deps.getCurrentTaskAgentType() !== 'multiagent' && isCurrentlyBrowserUse) {
          ...
          try { if (deps.sessionIdRef.current) chatHistoryStore.addMessage(deps.sessionIdRef.current, { actor: Actors.SYSTEM, content: finalText, timestamp } as any); } catch {}
        ...
        try {
          ...
          if (estimatorRootId) {
            deps.setMessageMetadata((prev: any) => ({ ...prev, [estimatorRootId]: { ...prev[estimatorRootId], isCompleted: true, workflowEndTime: timestamp } }));
        ...
        if (summary) {
          ...
          if (storeJobSummary(summary, taskId, 'task.ok', deps)) {
            ...
            if (!deps.lastAgentMessageRef.current && deps.agentTraceRootIdRef.current) {
              ...
              deps.logger.log('[Panel] Stored job summary for agent aggregate message:', deps.agentTraceRootIdRef.current);
            }
          }
        }
      ...
      case ExecutionState.TASK_CANCEL:
        ...
        deps.setActiveAggregateMessageId(null);
        try { if (deps.portRef.current?.name === 'side-panel-connection') deps.portRef.current.postMessage({ type: 'get-all-mirrors-for-cleanup' }); } catch {}
        try { if (deps.getWorkerTabGroups().length > 0 || deps.getCurrentTaskAgentType() === 'multiagent') deps.setShowCloseTabs(true); } catch {}
        deps.setMessages((prev: any[]) => prev.filter((msg: any, idx: number) => !(msg.content === 'Showing progress...' && idx === prev.length - 1)));
        persistFinalPreview(deps);
        try {
          ...
          if (!isAgentV2) {
            ...
            try {
              ...
              if (taskId) {
                ...
                if (deps.portRef.current?.name === 'side-panel-connection') deps.portRef.current.postMessage({ type: 'get_token_log', taskId });
              }
            } catch {}
          }
        } catch {}
        ...
        try { if (shouldShowCloseTabs(deps, data)) deps.setShowCloseTabs(true); } catch {}
        deps.setIsAgentModeActive(false);
        if (deps.agentTraceRootIdRef.current) {
          addTraceItem(Actors.SYSTEM, 'Task cancelled', timestamp, deps);
          markAggregateComplete(deps);
          try { if (deps.sessionIdRef.current) chatHistoryStore.addMessage(deps.sessionIdRef.current, { actor: Actors.SYSTEM, content: 'Task cancelled', timestamp } as any); } catch {}
        }
        try {
          const taskId = String(data?.taskId || deps.sessionIdRef.current || '');
          const estimatorRootId = taskIdToEstimatorRootId.get(taskId);
          ...
          if (data?.message) {
            ...
            if (payload?.type === 'request_user_control') {
              ...
              if (deps.agentTraceRootIdRef.current) {
                ...
                const messageId = `${timestamp}-${Actors.SYSTEM}`;
                deps.setMessageMetadata((prev: any) => ({ ...prev, [messageId]: { ...prev[messageId],
                  traceItems: [{ actor: Actors.SYSTEM, content: formattedMessage, timestamp, controlRequest: payload }] } }));
              }
            }
          }
        } catch {}
