...
export function ChatScreen(props: ChatScreenProps) {
  const {
    isDarkMode, messages, inputEnabled, showStopButton, isPaused, isHistoricalSession,
    currentSessionId, forceChatView, compactMode, sessionStats, requestSummaries,
    messageMetadata, mirrorPreview, mirrorPreviewBatch, hasFirstPreview, isPreviewCollapsed,
    agentTraceRootId, pendingEstimation, availableModelsForEstimation, showJumpToLatest,
    showCloseTabs, workerTabGroups, pinnedMessageIds, currentTaskAgentType, isJobActive,
    tokenLog, useFullPlanningPipeline, enablePlanner, enableValidator, useVisionState,
    hasConfiguredModels, favoritePrompts, replayEnabled, showEmergencyStop, isStopping, logger, portRef, sessionIdRef,
    agentTraceRootIdRef, panelRef, messagesEndRef, setInputTextRef, setSelectedAgentRef,
    setIsPreviewCollapsed, setSelectedEstimationModel, setRecalculatedEstimation,
    setPendingEstimation, setShowCloseTabs, setWorkerTabGroups, handleSendMessage,
    handleStopTask, handlePauseTask, handleResumeTask, handleReplay, handleClearChat,
    handleBookmarkSelect, handleBookmarkAdd, handleBookmarkUpdate, handleBookmarkDelete,
    handleBookmarkReorder, handlePinMessage, handleQuoteMessage, appendMessage, setupConnection,
    handleKillSwitch,
  } = props;
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      ...
      {hasConfiguredModels === null && messages.length === 0 && (
        <div className={`flex flex-1 items-center justify-center p-8 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
      ...
      {hasConfiguredModels === false && messages.length === 0 && (
        <div className={`flex flex-1 items-center justify-center p-8 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
          <div className="max-w-md text-center">
            ...
            <h3 className={`mb-2 text-3xl font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>Welcome to warpsurf!</h3>
            <p className="mb-4">To get started, please configure your API keys in the settings page.</p>
            <button onClick={() => chrome.runtime.openOptionsPage()} className={`my-4 rounded-lg px-4 py-2 font-medium transition-colors ${isDarkMode ? 'bg-violet-600 text-white hover:bg-violet-700' : 'bg-violet-500 text-white hover:bg-violet-600'}`}>
              Open Settings
            </button>
          </div>
        </div>
      ...
      {hasConfiguredModels === true && (
        <>
          ...
          {messages.length === 0 && !forceChatView && (
            <>
              <div className={`border-t ${isDarkMode ? 'border-slate-800' : 'border-gray-200'} mb-2 p-2 shadow-sm backdrop-blur-sm`}>
                <ChatInput
                  isDarkMode={isDarkMode} disabled={!inputEnabled || isHistoricalSession}
                  showStopButton={showStopButton} isPaused={isPaused} isJobActive={isJobActive}
                  isStopping={isStopping}
                  historicalSessionId={isHistoricalSession && replayEnabled ? currentSessionId : null}
                  onSendMessage={handleSendMessage} onStopTask={handleStopTask}
                  onPauseTask={handlePauseTask} onResumeTask={handleResumeTask} onReplay={handleReplay}
                  setContent={setter => { setInputTextRef.current = setter; }}
                  setAgentSelector={setter => { setSelectedAgentRef.current = setter; }}
                  onHandBackControl={(instructions) => {
                    ...
                    try {
                      if (portRef.current) {
                        ...
                        if (instructions && instructions.trim()) appendMessage({
                          actor: Actors.USER,
              ...
              <AvailableChatSection
                isDarkMode={isDarkMode} favoritePrompts={favoritePrompts}
                onExampleSelect={(content, agentType) => {
                  ...
                  } catch (e) { logger.error('Example select failed:', e); }
                }}
                onBookmarkSelect={handleBookmarkSelect} onBookmarkAdd={handleBookmarkAdd}
                onBookmarkUpdate={handleBookmarkUpdate} onBookmarkDelete={handleBookmarkDelete}
          ...
          {(messages.length > 0 || forceChatView) && (
            <>
              <div className={`relative flex-1 min-h-0 overflow-x-hidden overflow-y-auto p-2 messages-scroll ${isDarkMode ? 'bg-slate-900/80' : ''}`}>
                <MessageList
                  messages={messages} isDarkMode={isDarkMode} compactMode={compactMode}
                  jobSummaries={requestSummaries} metadataByMessageId={messageMetadata}
                  inlinePreview={hasFirstPreview ? mirrorPreview : null} inlinePreviewBatch={mirrorPreviewBatch}
                  isPreviewCollapsed={isPreviewCollapsed} activeAggregateMessageId={agentTraceRootId}
                  ...
                  onApproveEstimation={(selectedModel, updatedEstimation) => {
                    try {
                      ...
                      portRef.current?.postMessage({
                        type: 'approve_estimation', sessionId: currentSessionId,
                        selectedModel, estimation: updatedEstimation
                      });
                    } catch (e) { logger.error('[ChatScreen] Failed to send approval:', e); }
                  }}
                  onCancelEstimation={() => {
                    try {
                    ...
                    } catch (e) { logger.error('[ChatScreen] Failed to send cancellation:', e); }
                  }}
                  onTakeControl={(tabId) => {
                    try {
                      if (portRef.current) {
                  ...
                  onOpenPreviewTab={(tabId) => {
                    try {
                      ...
                      (async () => {
                        try {
                          if (!portRef.current) { setupConnection(); await new Promise(r => setTimeout(r, 120)); }
                  ...
                  pinnedMessageIds={pinnedMessageIds} onPinMessage={handlePinMessage}
              ...
              <div className={`border-t ${isDarkMode ? 'border-slate-800' : 'border-gray-200'} p-2 shadow-sm backdrop-blur-sm`}>
                <ChatInput
                  isDarkMode={isDarkMode} disabled={!inputEnabled || isHistoricalSession}
                  showStopButton={showStopButton} isPaused={isPaused} isJobActive={isJobActive}
                  isStopping={isStopping}
          ...
          {(messages.length > 0 || sessionStats.totalRequests > 0) && (
            <SessionControls
              isDarkMode={isDarkMode} sessionStats={sessionStats} formatUsd={formatUsd}
              onClearChat={handleClearChat} showCloseTabs={showCloseTabs} workerTabGroups={workerTabGroups}
              sessionIdForCleanup={currentSessionId || sessionIdRef.current}
              onClosedTabs={() => { setShowCloseTabs(false); setWorkerTabGroups([]); }}
              showEmergencyStop={showEmergencyStop}
              onEmergencyStop={handleKillSwitch}
            >
              <DebugButtons
                currentSessionId={currentSessionId} agentTraceRootIdRef={agentTraceRootIdRef}
                currentTaskAgentType={currentTaskAgentType} messageMetadata={messageMetadata}
                portRef={portRef} isDarkMode={isDarkMode} setErrorLogEntries={() => {}}
              />
            </SessionControls>
          )}
          {(messages.length > 0 || forceChatView) && (
