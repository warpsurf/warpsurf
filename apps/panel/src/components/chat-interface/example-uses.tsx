...
const examples: Array<{ title: string; content: string; agentType: AgentType; tint: 'violet' | 'teal' | 'amber' | 'black' | 'orange'; icon: JSX.Element }> = [
  {
  ...
  },
  {
    ...
    content: 'I want to go on a trip to Scandinavia. I don\'t know what the entry requirements for a British citizen are. Please open the UK government visa requirement websites for all relevant countries. Summarize the findings.',
    agentType: AgentType.MULTIAGENT,
    tint: 'orange',
    // two robots icons side by side not on separate lines
    icon: <><FaRobot className="h-3.5 w-3.5" /> <FaRobot className="h-3.5 w-3.5" /></>,
  },
];

export default function ExampleUses({ onSelect, isDarkMode = false }: ExampleUsesProps) {
  return (
    <div>
      <h3 className={`mb-2 text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>Example Uses</h3>
      ...
      <p className={`mt-2 text-[12px] ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Click to pre-fill; press Send to run.</p>
    </div>
  );
}
