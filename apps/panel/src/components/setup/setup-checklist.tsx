...
export default function SetupChecklist({
  ...
  return (
    <div
      className={`rounded-xl border p-4 shadow-sm ${isDarkMode ? 'border-slate-700 bg-slate-900/70 text-gray-200' : 'border-gray-200 bg-white/90 text-gray-800'}`}>
      ...
      <ol className="space-y-2 text-sm">
        <li className="flex items-center gap-2">
          ...
          <button
            ...
            className={`rounded px-2 py-1 text-xs ${isDarkMode ? 'bg-violet-500 text-white hover:bg-violet-400' : 'bg-violet-400 text-white hover:bg-violet-500'}`}>
            API Keys
          </button>
        </li>
        <li className="flex items-center gap-2">
      ...
      {!hasProviders && (
        <div
          className={`mt-3 flex items-center gap-2 rounded px-2 py-1 text-xs ${isDarkMode ? 'bg-amber-900/30 text-amber-200' : 'bg-amber-50 text-amber-700'}`}>
          <FaKey />
          <span>API keys are required to use models. Add one in Settings.</span>
        </div>
      )}
      <div
        className={`mt-3 flex items-center gap-2 rounded px-2 py-1 text-xs ${isDarkMode ? 'bg-sky-900/30 text-sky-200' : 'bg-sky-50 text-sky-700'}`}>
        <FaGlobe />
