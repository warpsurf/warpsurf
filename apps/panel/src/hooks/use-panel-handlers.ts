...
export function usePanelHandlers(params: {
  ...
  const { portRef, sessionIdRef, agentSettingsRef, setInputTextRef, setSelectedAgentRef, processedJobSummariesRef, lastAgentMessageRef, agentTraceActiveRef, isCancellingRef, cancelTimeoutRef, logger, showToast, stopConnection, setupConnection, appendMessage, loadChatSessions, handleSessionSelectHook, handleSessionBookmarkFromHook, resetPerChatAcceptance, promptPerChatIfEnabled, setMessages, setCurrentSessionId, setShowDashboard, setForceChatView, setInputEnabled, setShowStopButton, setIsFollowUpMode, setIsAgentModeActive, setCurrentTaskAgentType, setAgentTraceRootId, setMessageMetadata, setShowCloseTabs, setIsPaused, setMirrorPreview, setMirrorPreviewBatch, setHasFirstPreview, setSessionStats, setRequestSummaries, setIsHistoricalSession, setPinnedMessageIds, setActiveAggregateMessageId, setShowHistory, setIsJobActive, setShowPopulations, setFavoritePrompts, setAgentSettingsOpen, setHistoryContextLoading, setPaletteOpen, setIsStopping, workerTabGroups, currentTaskAgentType, pinnedMessageIds } = params;
  ...
  const handleNewChat = useCallback(() => {
    ...
    setSessionStats({ totalRequests: 0, totalInputTokens: 0, totalOutputTokens: 0, totalLatency: 0, totalCost: 0, avgLatencyPerRequest: 0 });
    setRequestSummaries({});
  ...
  }, [portRef, sessionIdRef, agentTraceActiveRef, processedJobSummariesRef, lastAgentMessageRef, resetPerChatAcceptance, promptPerChatIfEnabled, setMessages, setCurrentSessionId, setShowDashboard, setForceChatView, setInputEnabled, setShowStopButton, setIsFollowUpMode, setIsAgentModeActive, setCurrentTaskAgentType, setAgentTraceRootId, setMessageMetadata, setShowCloseTabs, setIsPaused, setMirrorPreview, setMirrorPreviewBatch, setHasFirstPreview, setSessionStats, setRequestSummaries, setIsHistoricalSession, stopConnection, setupConnection]);
  ...
  const handleStopTask = useCallback(async () => {
    ...
    try {
      ...
      appendMessage({ actor: Actors.SYSTEM, content: err instanceof Error ? err.message : String(err), timestamp: Date.now() });
      
      // On error, still update UI to allow retry
      setInputEnabled(true);
    }
  }, [portRef, sessionIdRef, isCancellingRef, cancelTimeoutRef, killSwitchFnRef, logger, appendMessage, setIsStopping, setInputEnabled]);
      ...
      if (sid) {
        ...
        try {
          ...
          await chrome.storage.local.set({
            agent_dashboard_running: (result.agent_dashboard_running || []).filter((a: any) => String(a?.sessionId) !== String(sid)),
            agent_dashboard_completed: (result.agent_dashboard_completed || []).filter((a: any) => String(a?.sessionId) !== String(sid)),
          });
        } catch {}
      }
  ...
  }, [sessionIdRef, logger, showToast, setMessages, setIsHistoricalSession, setShowHistory, setInputEnabled, setRequestSummaries, setMessageMetadata, setSessionStats, setPinnedMessageIds, setActiveAggregateMessageId, setHasFirstPreview, setMirrorPreview, setMirrorPreviewBatch, setShowCloseTabs, setIsJobActive, setForceChatView]);
  ...
  }, [setShowHistory, setShowDashboard, setShowPopulations, setCurrentSessionId, setMessages, setIsFollowUpMode, setIsHistoricalSession]);
  ...
  const handleBookmarkSelect = useCallback((content: string, agentType?: AgentType) => {
...
const handleOpenSettings = useCallback(async (tab?: 'api-keys' | 'workflow-settings' | 'web-settings' | 'warnings' | 'help') => {
    try {
  ...
  const handlePaletteSelect = useCallback((action: PaletteAction) => {
  ...
  const handleKillSwitchInternal = useCallback(async () => {
    ...
    const sendKill = (port: chrome.runtime.Port) => {
      try { port.postMessage({ type: 'kill_all' }); }
      catch { setIsJobActive(false); setInputEnabled(true); setShowStopButton(false); }
    };
    ...
    if (portRef.current) {
      ...
      try {
        ...
        sendKill(tempPort);
        setTimeout(() => { try { tempPort.disconnect(); } catch {} }, 1000);
      } catch {
        setIsJobActive(false); setInputEnabled(true); setShowStopButton(false);
      }
    }
  ...
  }, [portRef, appendMessage, showToast, processedJobSummariesRef, lastAgentMessageRef, agentTraceActiveRef, isCancellingRef, cancelTimeoutRef, setIsStopping, setIsJobActive, setInputEnabled, setShowStopButton]);
