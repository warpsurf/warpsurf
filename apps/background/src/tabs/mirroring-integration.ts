...
export async function startMirroringInit(args: MirrorArgs): Promise<void> {
  ...
  try {
    if (task.tabId && task.tabId !== tabId) {
      try {
        tabMirrorService.stopMirroring(task.tabId);
      } catch {}
    }
  } catch {}
  task.tabId = tabId;
  try {
    logger?.info?.(`Starting tab mirroring (init) for tab ${tabId}`);
  } catch {}
  // Ensure side panel port is set for mirror updates
  try {
    if (sidePanelPort) tabMirrorService.setDashboardPort(sidePanelPort);
  } catch {}
  if (!(generalSettings.showTabPreviews ?? true)) return;
  ...
  tabMirrorService.registerScreenshotProvider(tabId, async () => {
    try {
      ...
      try {
        logger?.error?.('Provider capture failed (init):', e);
      } catch {}
      return undefined;
    }
  });
  try {
    tabMirrorService.reserveDebuggerForPuppeteer(tabId, true);
  } catch {}
  tabMirrorService.startMirroring(tabId, task.id, task.color, task.parentSessionId || task.id, task.workerIndex);
  if (sidePanelPort) {
    setTimeout(() => {
      try {
        const mirrors: Array<any> =
          (tabMirrorService as any).getActiveMirrors?.() || (tabMirrorService as any).getCurrentMirrors?.() || [];
...
export async function startMirroringOnTabCreated(args: MirrorArgs): Promise<void> {
  ...
  tabMirrorService.registerScreenshotProvider(tabId, async () => {
    try {
      ...
      if (data) {
        try {
          logger?.info?.(`Screenshot captured for tab ${tabId}, size: ${data.length}`);
        } catch {}
        return `data:image/jpeg;base64,${data}`;
      }
      try {
        logger?.info?.(`No screenshot data returned for tab ${tabId}`);
      } catch {}
      return undefined;
    } catch (e) {
      try {
        logger?.error?.('Provider capture failed:', e);
      } catch {}
      return undefined;
    }
  });
  ...
  try {
    logger?.info?.(`Starting tab mirroring for tab ${tabId}`);
  } catch {}
  tabMirrorService.startMirroring(tabId, task.id, task.color, task.parentSessionId || task.id, task.workerIndex);
  if (sidePanelPort) {
    setTimeout(() => {
