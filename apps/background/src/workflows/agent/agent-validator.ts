...
export class AgentValidator extends BaseAgent<typeof validatorOutputSchema, ValidatorOutput> {
  ...
  async execute(): Promise<AgentOutput<ValidatorOutput>> {
    try {
      this.context.emitEvent(Actors.AGENT_VALIDATOR, ExecutionState.STEP_START, 'Validating...');
      ...
      if (!modelOutput.is_valid) {
        ...
        this.context.emitEvent(Actors.AGENT_VALIDATOR, ExecutionState.STEP_FAIL, msg);
        this.context.actionResults = [new ActionResult({ extractedContent: msg, includeInMemory: true })];
      } else {
        this.context.emitEvent(Actors.AGENT_VALIDATOR, ExecutionState.STEP_OK, modelOutput.answer);
      }
      ...
      if (isTimeoutError(error)) {
        ...
        logger.error(`Validation timeout: ${msg}`);
        this.context.emitEvent(Actors.AGENT_VALIDATOR, ExecutionState.STEP_FAIL, msg);
      ...
      this.context.emitEvent(Actors.AGENT_VALIDATOR, ExecutionState.STEP_FAIL, errorMessage);
      throw error;
    }
  }
}
