...
export class AgentPlanner extends BaseAgent<typeof plannerOutputSchema, PlannerOutput> {
  ...
  async execute(): Promise<AgentOutput<PlannerOutput>> {
    try {
      this.context.emitEvent(Actors.AGENT_PLANNER, ExecutionState.STEP_START, 'Planning...');
      ...
      if (isTimeoutError(error)) {
        ...
        logger.error(`Planning timeout: ${msg}`);
        this.context.emitEvent(Actors.AGENT_PLANNER, ExecutionState.STEP_FAIL, msg);
      ...
      this.context.emitEvent(Actors.AGENT_PLANNER, ExecutionState.STEP_FAIL, errorMessage);
      throw error;
    }
  }
}
