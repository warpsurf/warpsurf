/**
 * @deprecated Superseded by roles/captain.ts — the Captain handles prompt refinement during dispatch.
...
import { multiagentRefinerSystemPrompt } from '@src/workflows/multiagent/multiagent-refiner-prompt.deprecated';
...
function validateAndMergeRefinements(original: TaskPlan, candidate: any): TaskPlan {
  try {
    ...
    for (const ct of candSubtasks) {
      ...
      const noBrowse =
        ct.no_browse !== undefined ? !!ct.no_browse : ct.noBrowse !== undefined ? !!ct.noBrowse : !!base.noBrowse;
