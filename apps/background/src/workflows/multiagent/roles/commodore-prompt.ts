...
- Example: instead of Step 1 "Generate a list of 5 museums" → Step 2 "Find the website of the first museum from the list" (blocked on Step 1), you should directly create Step 1 "Find the website of the British Museum", Step 2 "Find the website of the V&A", etc. — all with no dependencies, running in parallel.
...
Task: "Find the websites for 5 UK national museums"

Note: The planner already knows 5 major UK national museums, so there is no need for a knowledge-only "generate list" subtask. The specific museums are embedded directly into the subtask prompts, allowing all lookups to run in parallel from the start.
...
"prompt": "Verify that all 5 museum websites were successfully opened and compile a summary listing each museum name with its URL for the user.",
"role": "validator"
}
}
