...
function normalizePlannerJson(raw: any): TaskPlan {
	...
	const subtasks: Subtask[] = subtasksRaw.map((t, idx) => {
		...
		const deps: SubtaskId[] = Array.isArray(t.dependencies) ? t.dependencies.map((d: any) => Number.parseInt(String(d), 10)).filter((n: number) => Number.isFinite(n)) : [];
		const noBrowse = !!(t.no_browse || t.noBrowse);
		const suggestedUrls: string[] = Array.isArray(t.suggested_urls) ? t.suggested_urls.map((s: any) => String(s)) : Array.isArray(t.suggestedUrls) ? t.suggestedUrls.map((s: any) => String(s)) : [];
		const suggestedSearchQueries: string[] = Array.isArray(t.suggested_search_queries) ? t.suggested_search_queries.map((s: any) => String(s)) : Array.isArray(t.suggestedSearchQueries) ? t.suggestedSearchQueries.map((s: any) => String(s)) : [];
 ...
 */
export async function planSubtasksFromQuery(query: string, llm: any, maxWorkers: number, signal?: AbortSignal, historyBlock?: string, sessionId?: string): Promise<TaskPlan> {
...
function optimizePlan(plan: TaskPlan): TaskPlan {
	try {
		...
		for (const s of subtasks) {
			...
			const looksLikeList = /(generate|list|provide|output)\b[\s\S]*\b(list|names|items|colleges|universities|examples)\b/.test(titleL + ' ' + promptL);
			if (looksLikeList) {
				s.noBrowse = true;
		...
		const isGenericSearch = (s: Subtask) => /google\s+search|web\s+search/.test(s.title.toLowerCase() + ' ' + s.prompt.toLowerCase());
