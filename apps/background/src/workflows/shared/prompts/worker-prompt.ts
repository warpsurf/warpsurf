import { commonSecurityRules, noPageContextGuidance } from './common';
...
export const siteSearchSection = isLegacyNavigation
  ? ''
  : `
...
export const siteSearchNavRule = isLegacyNavigation
  ? ''
  : `
...
/**
 * Shared base system prompt template for all browser automation agents
 * (single-agent navigator and multi-agent workers).
 *
 * Build-time placeholders (filled by consumer before constructing SystemMessage):
 *   {{role_preamble}}             — opening paragraph describing the agent's role
 *   {{site_search_section}}       — site search block before response rules (navigator only)
 *   {{site_search_guidance}}      — inline search guidance in actions section (navigator only)
 *   {{site_search_nav_rule}}      — navigation search rule (navigator only)
 *   {{vision_guidance_section}}   — vision/screenshot guidance (navigator only, runtime)
 *   {{region_preference_section}} — region preference (navigator only, runtime)
 *   {{post_sections}}             — additional sections appended after core rules
 *
 * Runtime placeholder (filled when constructing the prompt class):
 *   {{max_actions}}               — maximum actions per step
 */
export const workerBaseSystemPromptTemplate = `
<system_instructions>
{{role_preamble}}
...
- Navigation: [{"go_to_url": {"intent": "Go to url", "url": "https://example.com"}}]{{site_search_guidance}}
