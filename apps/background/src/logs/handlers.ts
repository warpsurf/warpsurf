...
export function handleGetAgentLog(port: chrome.runtime.Port, taskManager: any, taskId: string) {
  ...
  const usages = [
    ...
    ...tasks
      .filter((t: any) => String((t as any).parentSessionId || '') === taskId)
      .flatMap((t: any) => globalTokenTracker.getTokensForTask(String((t as any).id))),
  ];
  if (usages.length === 0) {
    ...
    for (const u of sorted) {
      ...
      apiLines.push(
        `- [${ts}] ${u.provider} • ${u.modelName} • in=${u.inputTokens} out=${u.outputTokens} total=${u.totalTokens} cost=$${(u.cost || 0).toFixed(6)}`,
      );
      try {
...
export function handleGetCombinedTokenLog(port: chrome.runtime.Port, taskManager: any, sessionId: string) {
  ...
  const scopeIds = [
    ...(main ? [String((main as any).id)] : [String(sessionId)]),
    ...related.map((t: any) => String((t as any).id)),
  ];
  const allUsages: any[] = [];
...
export function handleGetSessionLogs(port: chrome.runtime.Port, taskManager: any, sessionId: string) {
  ...
  try {
    ...
    return port.postMessage({
      ...
      error: (e as any)?.message || 'Failed to get session logs',
    });
  }
}
...
export function handleGetCombinedSessionLogs(port: chrome.runtime.Port, taskManager: any, sessionId: string) {
  ...
  try {
    ...
    const scopeIds: string[] = [
      ...(main ? [String((main as any).id)] : [sid]),
      ...related.map((t: any) => String((t as any).id)),
    ];
    ...
    // Get action schema for the session (logged once at workflow start)
    const schemaData = (globalTokenTracker as any)?.getSessionSchema?.(sid) || null;

    return port.postMessage({
      type: 'combined_session_logs',
      sessionId: sid,
      data: enriched,
      actionSchema: schemaData?.schema || null,
    });
  } catch (e) {
    return port.postMessage({
      type: 'combined_session_logs',
      sessionId: String(sessionId),
      error: (e as any)?.message || 'Failed to get combined session logs',
    });
  }
}
