...
async function runEstimationIfEnabled(
  ...
  logger.info(`[Estimation] Complete: ${estimation.steps.length} steps, ~${Math.round(estimation.summary.total_agent_duration_s)}s`);
  ...
  try {
    port?.postMessage({
      ...
      data: { taskId: sessionId, step: 0, maxSteps: 0, details: 'Workflow estimation ready', message: JSON.stringify(estimation) },
      timestamp: Date.now()
    });
  } catch {}
...
export async function handleNewTask(message: any, deps: Deps) {
  ...
  try {
    ...
    if (at === 'auto') {
      ...
      try { await (svc as any).initialize?.(); } catch {}
  ...
  try { await taskManager.createTask(task, taskName, true, sessionId, sessionId); } catch {}
    ...
    try {
      ...
      try {
        currentPort?.postMessage({
          ...
          data: { taskId: sessionId, step: 0, maxSteps: 0, details: `Estimation failed: ${error instanceof Error ? error.message : 'Unknown error'}` },
          timestamp: Date.now()
        });
      } catch {}
    }
  ...
  // For Agent path, optionally override tabId based on lightweight triage of "use current tab"
  try {
    if (isWebAgent) {
      ...
      if (!useCurrent) {
        ...
        try { browserContext.updateCurrentTabId(tabId); } catch {}
      }
    }
  } catch {}
  ...
  try {
    if (isWebAgent) {
      taskManager.setSingleAgentExecutor(sessionId, executor, tabId > 0 ? tabId : 0);
    } else {
  ...
  try { await subscribeToExecutorEvents(executor, currentPort, taskManager, deps.logger); } catch {}

  // Execute
  try { await executor.execute(); } catch (e: any) { try { currentPort?.postMessage({ type: 'error', error: e?.message || 'Task failed' }); } catch {} }
}
...
export async function handleFollowUpTask(message: any, deps: Deps) {
  ...
  if (existing && existingTaskId && String(existingTaskId) !== String(sessionId)) {
    logger.info(`[handleFollowUpTask] Executor belongs to different session (${existingTaskId} vs ${sessionId}), creating new executor`);
  ...
  if (existing && agentType === 'agent' && hasRunBrowserUse) {
      ...
      if (task) {
        ...
        if (currentTabId && currentTabId > 0) {
          ...
          (taskManager as any).tabMirrorService?.registerScreenshotProvider?.(captureTabId, async () => {
            try {
              const data = await existing.captureTabScreenshot?.(captureTabId) || await existing.captureCurrentPageScreenshot?.();
              return data ? `data:image/jpeg;base64,${data}` : undefined;
            } catch { return undefined; }
          });
          (taskManager as any).tabMirrorService?.startMirroring?.(captureTabId, task.id, task.color, task.parentSessionId || task.id, task.workerIndex);
    ...
    try { await existing.addBrowserUseFollowUpTask(task); } catch {}
    ...
    try { (existing as any).clearExecutionEvents?.(); } catch {}
    delete (existing as any).__backgroundSubscribed;
    try { await subscribeToExecutorEvents(existing, currentPort, taskManager, logger); } catch {}
    ...
    try { await existing.execute(); } catch (e: any) { 
      try { currentPort?.postMessage({ type: 'error', error: e?.message || 'Follow-up failed' }); } catch {} 
    }
    return;
  }
  ...
  if (existing && agentType !== 'agent') {
    ...
    try { existing.addFollowUpTask(task, agentType); } catch {}
