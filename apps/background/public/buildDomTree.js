window.buildDomTree = (
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
    ...
    if (
      ...
      if (node instanceof HTMLInputElement || node instanceof HTMLTextAreaElement) {
        ...
        // For contentEditable elements (like Google Docs), capture text content
        const text = (node.innerText || node.textContent || '').trim();
        if (text.length > 0) {
          // Store as special attribute so agent can see it
          nodeData.attributes['data-text-content'] = text.length > 200 ? text.slice(0, 200) + '...' : text;
        }
      }
    }
    ...
    if (node.tagName) {
      ...
      if (tagName === 'iframe') {
        try {
          ...
          // Silently skip iframes we can't access (cross-origin)
  ...
  // Clear existing highlights before building new tree to prevent stacking/duplication
