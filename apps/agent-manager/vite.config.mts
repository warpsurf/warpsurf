...
const panelSrcDir = resolve(rootDir, '..', 'panel', 'src');
...
function sharedIconsPlugin(): Plugin {
  return {
    ...
    writeBundle(options) {
      const outDir = options.dir || resolve(rootDir, '..', '..', 'dist', 'agent-manager');
...
export default withPageConfig({
  ...
  resolve: {
    alias: {
      '@src': srcDir,
      // Allow agent-manager to import panel's chat components directly for visual parity
      '@panel': panelSrcDir,
